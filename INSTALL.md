...
| Vercel | KV database (created via Vercel dashboard) | [vercel.com/dashboard/stores](https://vercel.com/dashboard/stores) |
...
# Vercel KV (auto-populated by vercel env pull after linking KV store)
...
| `KV_REST_API_URL` | Yes | Vercel KV connection URL (Redis-compatible store) |
...
**Security note:** `DEEPSEEK_API_KEY` is stored as a Vercel backend environment variable only. It is never exposed to the client. For local testing, `vercel env pull` writes it to `.env.local` (which is in `.gitignore`).
